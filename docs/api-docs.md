...
1. **Memory Layer** (`@luxbot/core/memory`) - Embeddings, extraction, storage, retrieval
2. **Database Operations** (`@luxbot/db`) - CRUD operations with vector search
...
### Embeddings
```typescript
import { embedText, embedBatch, cosineSimilarity } from '@luxbot/core';
...
### Extraction
```typescript
import { extractInfo, extractBatch, normalizeDueDate } from '@luxbot/core';
...
### Storage
```typescript
import { storeExtractedInfo, storeMemory, storeBatch } from '@luxbot/core';
...
### Retrieval
```typescript
import { searchExtractedInfo, searchMemory, multiSearch } from '@luxbot/core';
